...
export default function GetInModal({ onClose }: { onClose?: () => void }) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 409 && data.requiresLogin) {
        setIsEmailLoading(false);
        toast.info('This email already has an account. Please enter your password to log in.');
        
        // Store email for login and switch to unlock mode
        if (typeof window !== 'undefined') {
          sessionStorage.setItem('login_email', email);
        }
        
        // Switch to encrypted wallet unlock flow
        setTimeout(() => {
  ...
  const handleEncryptedWalletSubmit = async (password: string, email?: string) => {
    try {
      // If email is provided, it's a login attempt
      if (email && encryptedWalletMode === 'unlock') {
        const { loginWithEmailAndPassword } = await import('@/lib/emailPasskeyLogin');
        ...
        if (result.success && result.address) {
          ...
          } else if (errorMessage.toLowerCase().includes('not found') || errorMessage.toLowerCase().includes('no wallet')) {
        ...
        if (walletInfo) {
          toast.success('¡Billetera desbloqueada!');
      ...
      if (errorMessage.toLowerCase().includes('invalid password') || errorMessage.toLowerCase().includes('password')) {
        toast.error('Contraseña incorrecta. Verifica tu contraseña e inténtalo de nuevo.');
      } else if (errorMessage.toLowerCase().includes('not found')) {
        toast.error('Cuenta no encontrada. Verifica tu correo electrónico.');
      } else {
        toast.error('Error al autenticar. Por favor, inténtalo de nuevo.');
      }
    }
  };
