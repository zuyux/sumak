...
;; Test Version - NFT_NAME placeholder filled
...
(begin
  (unwrap! (nft-mint? test-nft u1 DEPLOYER) (err ERR-NFT-MINT))
  (map-set cids u1 "ipfs://QmInitialTest")
...
(define-public (set-token-uri (token-id uint) (metadata-cid (string-ascii 256)))
  (begin
    ...
    (asserts! (is-some (nft-get-owner? test-nft token-id)) (err ERR-NOT-FOUND))
...
(define-public (mint-additional (metadata-cid (string-ascii 256)))
  (let ((token-id (+ (var-get last-id) u1)))
    (asserts! (not (var-get locked)) (err ERR-CONTRACT-LOCKED))
    (try! (nft-mint? test-nft token-id tx-sender))
...
(define-public (buy-in-sat (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? test-nft id) (err ERR-NOT-FOUND)))
...
(define-private (is-owner (token-id uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? test-nft token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? test-nft id sender recipient)
    success
      (begin
